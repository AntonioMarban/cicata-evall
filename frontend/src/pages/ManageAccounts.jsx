...
const AccountTypeButton = ({ name, selectedName, onClick }) => {
  ...
  return (
    <button
      ...
      style={{ padding: '10px 20px', textAlign: 'center', margin: '0px', whiteSpace: 'nowrap', minWidth: '250px' }}
      onClick={handleClick}
    >
      {name}
    </button>
  ...
  return (
    <div className='flex flex-col min-h-screen' style={{ padding: '5%' }}>
      <div id="manageAccountsHead" className="flex w-full max-w-4xl justify-between items-start flex-col mb-6">
        ...
        <p className="text-gray-600 text-2xl mb-2!">Tu panel de administración de cuentas del sistema:</p>
      </div>
      ...
      {userType === 2 && (
        <div id="manageAccountsButtons" style={{ padding: '3% 0%' }}>
          <div className="flex flex-row flex-wrap gap-4 m-0!">
            ...
            <AccountTypeButton name="Presidentes de comité" selectedName={selectedPanel} onClick={handlePanelClick} />
            <AccountTypeButton name="Secretarios de comité" selectedName={selectedPanel} onClick={handlePanelClick} />
          </div>
        </div>
      )}
