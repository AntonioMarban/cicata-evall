...
const ShowCardsCollab = ({cards,handleDeleteFile,handleEditModal,nameArray,setData}) => {  
        ...
        {Array.isArray(cards) && cards.map((card, index) => (
            <div className="!p-2 m-5 flex justify-between items-center w-full" key={index}>
                ...
                <p className="flex-1 flex">
                    <p>{card.collaborationAgreement}</p>
                </p>

                <p className="flex-1 flex justify-center">
                    <p>{card.agreementType}</p>
                </p>

                <p className="flex-1 flex justify-center">{card.agreementType}</p>
...
export default ShowCardsCollab;
